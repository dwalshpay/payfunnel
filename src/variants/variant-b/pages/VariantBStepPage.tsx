...
import { VARIANT_B_STEPS, VARIANT_B_TOTAL_STEPS } from '../data/variantBStepConfig';
...
export function VariantBStepPage() {
  ...
  const { currentStep, answers, isLoading, transitionMessage, pointsAnimation } = state;
  ...
  // Apply smart defaults when entering step 3 (after industry selection)
  useEffect(() => {
    if (currentStep === 2 && answers.industry) {
      applySmartDefaults(answers.industry);
    }
  }, [currentStep, answers.industry, applySmartDefaults]);

  const handleContinue = () => {
    if (currentStep === VARIANT_B_TOTAL_STEPS - 1) {
      completeFunnel();
    } else {
      ...
      nextStep(message, stepConfig.pointsAwarded);
    }
  };
  ...
  const handleSingleSelect = (field: string, value: string) => {
    ...
    if (currentStep === 0) {
      setTimeout(() => {
        nextStep(stepConfig.transitionMessage, stepConfig.pointsAwarded);
      }, 300);
    }
  };
  ...
  return (
    <>
      ...
      <TransitionMessage
        ...
        pointsAwarded={pointsAnimation.show ? pointsAnimation.amount : undefined}
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-[#F5F5F5] overflow-hidden">
        ...
        <div className="px-8 pt-6 pb-4 border-b border-[#F5F5F5]">
          <StepIndicator currentStep={currentStep} totalSteps={VARIANT_B_TOTAL_STEPS} />
        </div>
