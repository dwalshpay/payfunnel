...
import { VARIANT_BA_TOTAL_STEPS, INDUSTRY_DEFAULTS } from '../data/variantBAStepConfig';
...
// State types
export interface VariantBAAnswers {
  // Step 1
  role: string | null;
  // Step 2
...
export interface VariantBAState {
  currentStep: number;
...
// Action types
type VariantBAAction =
  | { type: 'SET_SINGLE_ANSWER'; field: keyof VariantBAAnswers; value: string }
  | { type: 'TOGGLE_MULTI_ANSWER'; field: keyof VariantBAAnswers; value: string }
...
// Initial state
const initialAnswers: VariantBAAnswers = {
  role: null,
  industry: null,
  employees: null,
  paymentMethods: [],
...
const initialState: VariantBAState = {
  currentStep: 0,
...
// Reducer
function variantBAReducer(state: VariantBAState, action: VariantBAAction): VariantBAState {
  switch (action.type) {
    ...
    case 'TOGGLE_MULTI_ANSWER': {
      const field = action.field as keyof VariantBAAnswers;
    ...
    case 'NEXT_STEP':
      return {
        ...state,
        currentStep: Math.min(state.currentStep + 1, VARIANT_BA_TOTAL_STEPS - 1),
    ...
    case 'GO_TO_STEP':
      return {
        ...state,
        currentStep: Math.max(0, Math.min(action.step, VARIANT_BA_TOTAL_STEPS - 1)),
        transitionMessage: null,
      };
...
// Context
interface VariantBAContextValue {
  state: VariantBAState;
  // Setters
  setSingleAnswer: (field: keyof VariantBAAnswers, value: string) => void;
  toggleMultiAnswer: (field: keyof VariantBAAnswers, value: string) => void;
...
const VariantBAContext = createContext<VariantBAContextValue | null>(null);

// Provider
export function VariantBAProvider({ children }: { children: ReactNode }) {
  const [state, dispatch] = useReducer(variantBAReducer, initialState);

  const setSingleAnswer = useCallback((field: keyof VariantBAAnswers, value: string) => {
    dispatch({ type: 'SET_SINGLE_ANSWER', field, value });
  }, []);

  const toggleMultiAnswer = useCallback((field: keyof VariantBAAnswers, value: string) => {
  ...
  const getStepProgress = useCallback(() => {
    return {
      ...
      total: VARIANT_BA_TOTAL_STEPS,
      percent: ((state.currentStep + 1) / VARIANT_BA_TOTAL_STEPS) * 100,
    };
  }, [state.currentStep]);

  return (
    <VariantBAContext.Provider
      value={{
        state,
        setSingleAnswer,
        toggleMultiAnswer,
...
// Hook
export function useVariantBA() {
  const context = useContext(VariantBAContext);
  if (!context) {
    throw new Error('useVariantBA must be used within a VariantBAProvider');
  }
  return context;
}
