...
import { VARIANT_BA_STEPS, VARIANT_BA_TOTAL_STEPS } from '../data/variantBAStepConfig';
...
export function VariantBAStepPage() {
  const {
    state,
    setSingleAnswer,
    toggleMultiAnswer,
  ...
  const stepConfig = VARIANT_BA_STEPS[currentStep];
  ...
  const handleContinue = () => {
    if (currentStep === VARIANT_BA_TOTAL_STEPS - 1) {
      completeFunnel();
    } else {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border border-[#F5F5F5] overflow-hidden">
        ...
        <div className="px-8 pt-6 pb-4 border-b border-[#F5F5F5]">
          <StepIndicator currentStep={currentStep} totalSteps={VARIANT_BA_TOTAL_STEPS} />
        </div>
